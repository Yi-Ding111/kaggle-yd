...
from common.audioprocess import rating_value_interplote, audio_weight
from common.audiotransform import (
    CustomCompose,
    CustomOneOf,
    NoiseInjection,
    GaussianNoise,
...
model = timm.create_model(
    "tf_efficientnetv2_s_in21k", pretrained=True, in_chans=3
)  # can change the data channel accepted by the pre-trained model by passing in argument in_chans
...
feature_extractor = torch.nn.Sequential(
    *list(model.children())[
        :-3
    ]  # Remove the last three layers, which needs to be adjusted according to the actual model structure
)
...
class BirdclefDataset(Dataset):
    ...
    def get_audio_path(self, file_name: str) -> str:
        """
        Get the audio path of the corresponding index through the provided train metadata csv file. Since there is only one index, only one path will be returned.
    ...
    def __getitem__(self, index):
        ...
        # augmentation
        # only for train df
        # do not do augmentation on val set
        if self.train:
...
def trainloader_collate(batch):
    """
    Define how each batch should be stacked when creating data batches
    parameters:
    ...
    # Use Compose to combine multiple audio transformation operations. These operations are applied to the input audio data to improve the generalization and robustness of the model.
